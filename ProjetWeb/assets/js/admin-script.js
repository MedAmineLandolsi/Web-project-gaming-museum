...
// Add active state to nav items (without blocking navigation)
...
function updateActiveNavFromLocation() {
    if (!navItems || navItems.length === 0) return;

    const currentHash = window.location.hash || '';

    // Prefer matching by hash if present
    if (currentHash) {
        const matchByHash = Array.from(navItems).find(a => {
            const href = a.getAttribute('href') || '';
            return href.includes(currentHash);
        });
        if (matchByHash) {
            setActiveNavItem(matchByHash);
            return;
        }
    }

    // Fallback: match by pathname+search (ignoring hash)
    const currentPathAndSearch = window.location.pathname + window.location.search;
    const matchByPath = Array.from(navItems).find(a => {
        const href = a.getAttribute('href') || '';
        const hrefNoHash = href.split('#')[0];
        return hrefNoHash && currentPathAndSearch.endsWith(hrefNoHash);
    });

    if (matchByPath) setActiveNavItem(matchByPath);
}
...
function sortTable(columnIndex, tableId = 'reclamationsTable') {
...
function exportTableToCSV(tableId = 'reclamationsTable', filename = 'reclamations_export.csv') {
    const table = document.getElementById(tableId);
    if (!table) return;
...
function printTable(tableId = 'reclamationsTable') {
    const table = document.getElementById(tableId);
    if (!table) return;
    ...
    printWindow.document.write('<h2>Réclamations</h2>');
