...
class Reponse {
    ...
    public function getAll() {
        $query = "SELECT rep.*, u.username AS user_username, u.email AS user_email
                  FROM " . $this->table_name . " rep
    ...
    private function add($reponse) {
        $query = "INSERT INTO " . $this->table_name . " 
                  SET reclamationId=:reclamationId, user_id=:user_id, message=:message, 
                      adminName=:adminName";
    ...
    private function update($reponse) {
        $query = "UPDATE " . $this->table_name . " 
                  SET user_id=:user_id, message=:message, adminName=:adminName, date_reponse=NOW()
    ...
    public function getByReclamationId($id) {
        $query = "SELECT rep.*, u.username AS user_username, u.email AS user_email,
                         DATE_FORMAT(rep.date_reponse, '%d/%m/%Y') as dateReponse,
                         DATE_FORMAT(rep.date_reponse, '%H:%i:%s') as heureReponse
                  FROM " . $this->table_name . " rep
                  LEFT JOIN users u ON u.id = rep.user_id
                  WHERE rep.reclamationId = :id";
        $stmt = $this->conn->prepare($query);
