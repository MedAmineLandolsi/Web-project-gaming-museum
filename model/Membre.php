...
class Membre {
    ...
    public function create() {
        $query = "INSERT INTO " . $this->table . " 
                  SET nom=:nom, prenom=:prenom, email=:email, mot_de_passe=:mot_de_passe, 
                      statut=:statut, avatar=:avatar, bio=:bio, user_id=:user_id";
    ...
    // Trouver un membre par user_id (référence vers table users)
    public function findByUserId($user_id) {
        $query = "SELECT id FROM " . $this->table . " WHERE user_id = :user_id LIMIT 0,1";
        $stmt = $this->conn->prepare($query);
